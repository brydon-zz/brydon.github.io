...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title><? if (!isset($title)) { echo "Articles";}else { echo $title;} ?> - Brydon Eastman Assignment CSC241</title>
		...
		<script type="application/javascript" src="comment.js"></script>
	</head>
...
<br /><?if($admin==1){?><a href='publish.php' title='Create an Article'>Create</a><?
if(isset($article)){?> 
<a href='publish.php?a=<?=$_GET['a']?>' title='Edit an Article'>Edit</a>
<br />
<?}}?>
<a href='#' id='logout'>Logout</a><?}else{?><a href="signup.php" title="Sign Up">Sign Up</a> / <a id="loginA" title="Login">Login</a><br />
<div id="loginform">
		...
		<div id="cont">
			...
			<div id="mid">
				<div id="midcont">
					<div id="leftmid">
						<div id="article">
							<? if(isset($article)) { echo $article; } else if(!isset($_GET['a'])) { ?><h1 id="edit">Read an Article</h1>
							<ul>
								<?foreach($db->query("SELECT title,id FROM articles") as $t) {?>
								<li><a href="articles.php?a=<?=$t['id']?>" title="<?=$t['title']?>"><?=$t['title']?></a></li>
								<?}?>
							</ul>
 <? } else { ?><p>This article does not exist. Stop making things up.</p><?}?>
							<hr />
							<div id="cmntForm" class=<? if (isset($article)) echo $a; else echo 0; ?>>
								<?if (isset($article)) {?>
								<? if (isset($_SESSION['username'])) {?>
								<p>Enter a comment in the textarea below. Hit enter when you are done or shift+enter to make a newline.</p>
								<textarea id="comment" maxlength="400"></textarea><br />
								<? } else { ?>
				...
				<div id="footcont">
					<div id="botcont">
						...
						<div class="last">
							<h1>CSS Themes</h1>
							<ul>
								<li><a  onclick="changeCSS(0)">Default CSS</a></li>
								<li><a  onclick="changeCSS(1)">No CSS</a></li>
							</ul>
						</div>
					</div>
				</div>
